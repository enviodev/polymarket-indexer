import { describe, it, expect } from "vitest";
import { computePositionId } from "../src/common/utils/ctf-util";

describe("computePositionId", () => {
  it.only("It should compute the positionId", () => {
    const collateralAddress: `0x${string}` =
      "0x7D1DC38E60930664F8cBF495dA6556ca091d2F92";
    const conditionId: `0x${string}` =
      "0xda558eddf6eb57760bd5371fb313167f871d823a16e9d66fccb292baf2a117c0";

    const expectedPositionId0 =
      "108051088633899060239124498527429950692254744883563327407154880807410490438693";
    const expectedPositionId1 =
      "45163082656174410071592939534766820181648934703824597457997612898109272294349";

    const computedPositionId0 = computePositionId(
      collateralAddress,
      conditionId,
      0,
    );
    const computedPositionId1 = computePositionId(
      collateralAddress,
      conditionId,
      1,
    );

    expect(computedPositionId0.toString()).toBe(expectedPositionId0);
    expect(computedPositionId1.toString()).toBe(expectedPositionId1);
  });
});
